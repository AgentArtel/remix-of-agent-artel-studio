...
export const CredentialModal: React.FC<CredentialModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div 
        className={cn('relative w-full max-w-lg bg-dark-100 border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-scale-in', className)}
      ...
      >
        ...
        <div className="p-6 space-y-5">
          {!editingCredential && (
            <FormSelect
              ...
              value={selectedType}
              options={credentialTypes.map(t => ({ value: t.id, label: t.name }))}
              onChange={setSelectedType}
            />
          )}
          <FormInput label="Credential Name" value={name} onChange={setName} placeholder="e.g., My OpenAI Account" />
          ...
          {testResult && (
            <div className={cn('flex items-center gap-2 px-4 py-3 rounded-xl', testResult === 'success' ? 'bg-green/10' : 'bg-danger/10')}>
              <Check className={cn('w-4 h-4', testResult === 'success' ? 'text-green' : 'text-danger')} />
              <span className={cn('text-sm', testResult === 'success' ? 'text-green' : 'text-danger')}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 bg-dark-200/30">
          <button onClick={handleTest} disabled={isTesting} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm text-white/60 hover:text-white hover:bg-white/5 transition-colors disabled:opacity-50">
            {isTesting ? (
