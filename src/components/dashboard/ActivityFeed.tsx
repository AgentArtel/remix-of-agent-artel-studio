...
export const ActivityFeed: React.FC<ActivityFeedProps> = ({
  ...
  const getIconBg = (type: ActivityItem['type']) => {
    switch (type) {
      case 'execution': return 'bg-blue-400/15 text-blue-400';
      case 'success': return 'bg-green/15 text-green';
      case 'error': return 'bg-danger/15 text-danger';
      case 'created': return 'bg-warning/15 text-warning';
      case 'updated': return 'bg-white/10 text-white/60';
    }
  };
        ...
        {activities.map((activity, index) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-white/80 italic">{activity.message}</p>
              <p className="text-xs text-white/40 mt-0.5 italic">{activity.workflowName}</p>
            </div>
            <span className="text-xs text-white/30 whitespace-nowrap italic">{activity.timestamp}</span>
          </div>
        ))}
      </div>
    </div>
