    ...
    >
      <div className="relative h-40 bg-dark-200 overflow-hidden">
        ...
        <div className="absolute top-3 right-3">
          <Chip variant={difficultyColors[difficulty]} size="sm">{difficulty}</Chip>
        </div>
      </div>

      <div className="p-5">
        <h4 className="text-base font-medium text-white mb-2 italic">{name}</h4>
        <p className="text-sm text-white/50 line-clamp-2 mb-4 italic">{description}</p>
        
        <div className="flex items-center justify-between">
          <span className="text-xs text-white/40 italic">{nodes} nodes</span>
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" className="text-white/60 hover:text-white" onClick={onPreview}>Preview</Button>
            <Button size="sm" className="bg-green text-dark hover:bg-green-light" onClick={onUse}>
