 ...
 * ============================================================================
 * Production-ready workflow editor with save/load to Supabase, execution
 * persistence, n8n-aligned node types, and inline name editing.
 *
 * @author Open Agent Artel Team
 * @version 5.0.0 (Track A — Supabase persistence)
 * ============================================================================
 */
...
import {
  ...
  UNDO_SHORTCUT, REDO_SHORTCUT, REDO_ALT_SHORTCUT,
  DELETE_SHORTCUT, BACKSPACE_DELETE_SHORTCUT,
  SELECT_ALL_SHORTCUT, ESCAPE_SHORTCUT, SAVE_SHORTCUT,
...
import { Copy, Scissors, Trash2, Settings, Plus, Check, X, Loader2 } from 'lucide-react';
...
// Trigger node types that cannot have incoming connections
const TRIGGER_TYPES: NodeType[] = ['trigger', 'webhook', 'schedule'];
...
export const WorkflowEditorPage: React.FC<WorkflowEditorPageProps> = ({ onNavigate, initialWorkflowId }) => {
  ...
  // ===========================================================================
  const [workflowId, setWorkflowId] = useState<string>(() => initialWorkflowId || crypto.randomUUID());
  ...
  // ===========================================================================
  const {
    nodes, connections, pushState, undo, redo, canUndo, canRedo, reset,
  } = useUndoRedo({
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        const loadedNodes = (data.nodes_data as unknown as NodeData[]) ?? [];
        const loadedConns = (data.connections_data as unknown as Connection[]) ?? [];
        if (loadedNodes.length > 0) {
          reset({ nodes: loadedNodes, connections: loadedConns });
        }
        setIsLoadingWorkflow(false);
      });
  }, [initialWorkflowId]);
  ...
  // ===========================================================================
  const {
    selectedNodeIds, selectedConnectionIds, selectionBox,
    isNodeSelected, isConnectionSelected, hasSelection,
    handleNodeClick: handleSelectionNodeClick,
    handleConnectionClick: handleSelectionConnectionClick,
    handleSelectionBoxStart, handleSelectionBoxMove, handleSelectionBoxEnd,
    selectAll, clearSelection,
  } = useSelection({
    onSelectionChange: (nodeIds, connectionIds) => {
      if (nodeIds.size + connectionIds.size !== 1) setIsConfigPanelOpen(false);
    },
  });
  ...
  // CONTEXT MENU
  // ===========================================================================
  const { menuState, position: menuPosition, openMenu, closeMenu } = useContextMenu();

  // ===========================================================================
  // TOAST NOTIFICATIONS
  // ===========================================================================
  const { toasts, success: showSuccess, error: showError, info: showInfo, dismiss: dismissToast } = useToast({ defaultDuration: 4000 });
  ...
  const {
    progress, isExecuting, executionState, nodeResults,
    startExecution, stopExecution, getNodeStatus,
  } = useExecution({
    nodes,
    connections,
    onExecutionStart: () => {
    ...
    onExecutionComplete: async (result) => {
      ...
      if (currentExecutionId) {
        await supabase.from('studio_executions').update({
          status: result.status === 'completed' ? 'success' : 'error',
          completed_at: new Date().toISOString(),
          duration_ms: result.duration,
          node_results: result.nodeStatuses as any,
        }).eq('id', currentExecutionId);
        // Update workflow stats
        await supabase.from('studio_workflows').update({
          last_run_at: new Date().toISOString(),
          execution_count: undefined, // increment handled below
        }).eq('id', workflowId);
        // Increment execution count
        const { data: wf } = await supabase.from('studio_workflows').select('execution_count').eq('id', workflowId).single();
        if (wf) {
          await supabase.from('studio_workflows').update({
  ...
  const handleImportComplete = useCallback((result: N8nImportResult, envVars?: Record<string, string>) => {
    ...
    if (envVars) console.log('[n8nImport] Environment variables collected:', envVars);
  ...
  const getNodePosition = useCallback((node: NodeData) => {
    if (draggedNodeId === node.id && liveNodePositions[node.id]) return liveNodePositions[node.id];
  ...
  // ===========================================================================
  const {
    startConnection, handleMouseMove: handleConnectionMouseMove, endConnection,
    tempLine, isDrawing: isConnectionDrawing, snappedPort, compatiblePorts,
    ...
    onConnectionCreate: useCallback((connection: Connection) => {
      // Block connections INTO trigger nodes
      const targetNode = nodes.find(n => n.id === connection.to);
      if (targetNode && TRIGGER_TYPES.includes(targetNode.type)) {
        showError('Trigger nodes cannot have inputs');
        return;
      }
      pushState({ nodes, connections: [...connections, connection] }, 'add-connection');
      showSuccess('Connection created');
    }, [nodes, connections, pushState, showSuccess, showError]),
  });
  ...
  useEffect(() => {
    return registerShortcuts([
      { ...UNDO_SHORTCUT, handler: () => { if (canUndo) { undo(); showInfo('Undo'); return true; } return false; } },
      { ...REDO_SHORTCUT, handler: () => { if (canRedo) { redo(); showInfo('Redo'); return true; } return false; } },
      { ...REDO_ALT_SHORTCUT, handler: () => { if (canRedo) { redo(); showInfo('Redo'); return true; } return false; } },
      { ...DELETE_SHORTCUT, handler: () => { handleDeleteSelected(); return true; } },
      { ...BACKSPACE_DELETE_SHORTCUT, handler: () => { handleDeleteSelected(); return true; } },
      { ...SELECT_ALL_SHORTCUT, handler: () => { selectAll(nodes, connections); showInfo('All items selected'); return true; } },
      { ...ESCAPE_SHORTCUT, handler: () => { clearSelection(); setIsConfigPanelOpen(false); setIsNodePaletteOpen(false); closeMenu(); return true; } },
      { ...SAVE_SHORTCUT, handler: () => { handleSaveWorkflow(); return true; } },
    ]);
  ...
  const { touchHandlers, isTouchDevice } = useTouchSupport({
    enabled: true,
    onTap: () => {},
    onDoubleTap: () => { if (selectedNodeIds.size === 1) setIsConfigPanelOpen(true); },
    onLongPress: (longX, longY, target) => {
      const nodeEl = (target as HTMLElement)?.closest('[data-node-id]');
      if (nodeEl) {
        const nodeId = nodeEl.getAttribute('data-node-id');
        if (nodeId) openMenu('node', longX, longY, nodeId);
      } else {
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isSelectingRef.current) {
      const canvasPos = screenToCanvas(e.clientX, e.clientY, getCanvasTransform(canvasRef.current));
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent, type: 'node' | 'connection' | 'canvas', targetId?: string) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handlePortMouseDown = useCallback((e: React.MouseEvent, portId: string, portType: PortType) => {
    ...
    if (nodeId) startConnection(e, nodeId, portId, portType);
  }, [startConnection]);

  const handlePortMouseUp = useCallback((e: React.MouseEvent, portId: string, _portType: PortType) => {
    const nodeElement = (e.target as HTMLElement).closest('[data-node-id]');
    const nodeId = nodeElement?.getAttribute('data-node-id');
    if (nodeId) endConnection(e, nodeId, portId);
  }, [endConnection]);
  ...
  const handleAddNode = useCallback((nodeType: NodeType) => {
    const titleMap: Record<string, { title: string; subtitle: string }> = {
      ...
      'anthropic-chat': { title: 'Claude', subtitle: 'Anthropic Claude' },
      ...
      'schedule': { title: 'Schedule', subtitle: 'Cron trigger' },
      'memory': { title: 'Memory', subtitle: 'Chat memory' },
      'gmail': { title: 'Gmail', subtitle: 'Email actions' },
      'slack': { title: 'Slack', subtitle: 'Messaging' },
    ...
    if (isToolType || isMemoryType) {
      ...
      if (agents.length > 0) {
        ...
        for (const agent of agents) {
          ...
          if (dist < nearestDist) { nearestDist = dist; nearestAgent = agent; }
        }
        const targetPort = isMemoryType ? 'memory' : 'tool';
        const alreadyConnected = connections.some(c => c.to === nearestAgent.id && c.toPort === targetPort && c.fromPort === 'output');
    ...
    pushState({ nodes: [...nodes, newNode], connections: newConnections }, 'add-node');
  ...
  const handleDeleteSelected = useCallback(() => {
    ...
    const newConnections = connections.filter(
      (c) => !selectedConnectionIds.has(c.id) && !selectedNodeIds.has(c.from) && !selectedNodeIds.has(c.to)
    );
  ...
  const handleDeleteNode = useCallback(() => {
    ...
    const newNodes = nodes.filter((n) => !selectedNodeIds.has(n.id));
    const newConnections = connections.filter((c) => !selectedNodeIds.has(c.from) && !selectedNodeIds.has(c.to));
  ...
  // ===========================================================================
  // WORKFLOW RUN — calls studio-run edge function first, falls back to local
  // ===========================================================================
  const handleRunWorkflow = useCallback(async () => {
    ...
    // Try n8n bridge first
    try {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/studio-run`, {
        method: 'POST',
        headers: {
      ...
      if (data.mode === 'n8n' && data.success) {
        showSuccess('Workflow ran via n8n');
        await supabase.from('studio_executions').update({
          status: 'success',
          completed_at: new Date().toISOString(),
          node_results: data.result as any,
        }).eq('id', execId);
        setCurrentExecutionId(null);
        return;
      }
  ...
  }, [nodes, connections, workflowId, workflowName, showSuccess, showError]);
  ...
  const getNodeMenuItems = useCallback((nodeId: string) => {
    ...
    if (!node) return [];
    return [
      { id: 'configure', label: 'Configure', icon: <Settings className="w-4 h-4" />, onClick: () => { handleSelectionNodeClick(nodeId); setIsConfigPanelOpen(true); } },
      { id: 'duplicate', label: 'Duplicate', icon: <Copy className="w-4 h-4" />, shortcut: 'Ctrl+D', onClick: () => {
        const newNode = { ...node, id: `node-${Date.now()}`, position: { x: node.position.x + 50, y: node.position.y + 50 } };
        pushState({ nodes: [...nodes, newNode], connections }, 'duplicate-node');
        showSuccess('Node duplicated');
      }},
      MenuPresets.divider('div-1'),
      { id: 'copy', label: 'Copy', icon: <Copy className="w-4 h-4" />, shortcut: 'Ctrl+C', onClick: () => showInfo('Copied to clipboard') },
      { id: 'cut', label: 'Cut', icon: <Scissors className="w-4 h-4" />, shortcut: 'Ctrl+X', onClick: () => showInfo('Cut to clipboard') },
      MenuPresets.divider('div-2'),
      { id: node.isDeactivated ? 'activate' : 'deactivate', label: node.isDeactivated ? 'Activate' : 'Deactivate', icon: node.isDeactivated ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />, onClick: () => {
        const newNodes = nodes.map((n) => n.id === nodeId ? { ...n, isDeactivated: !n.isDeactivated } : n);
        pushState({ nodes: newNodes, connections }, 'toggle-activation');
        showInfo(node.isDeactivated ? 'Node activated' : 'Node deactivated');
      }},
      { id: 'delete', label: 'Delete', icon: <Trash2 className="w-4 h-4" />, shortcut: 'Del', onClick: () => {
  ...
  const getCanvasMenuItems = useCallback(() => [
    { id: 'add-node', label: 'Add Node', icon: <Plus className="w-4 h-4" />, onClick: () => setIsNodePaletteOpen(true) },
    MenuPresets.divider('div-1'),
    { id: 'select-all', label: 'Select All', shortcut: 'Ctrl+A', onClick: () => { selectAll(nodes, connections); showInfo('All items selected'); } },
    { id: 'deselect-all', label: 'Deselect All', onClick: () => { clearSelection(); showInfo('Selection cleared'); } },
    MenuPresets.divider('div-2'),
    { id: 'fit-to-view', label: 'Fit to View', onClick: () => showInfo('Fit to view') },
    { id: 'reset-view', label: 'Reset View', onClick: () => showInfo('View reset') },
  ...
  const getConnectionPositions = (connection: Connection) => {
    ...
    if (!fromNode || !toNode) return { fromPos: { x: 0, y: 0 }, toPos: { x: 0, y: 0 } };
    const fromPosition = getNodePosition(fromNode);
    const toPosition = getNodePosition(toNode);
    const fromPos = getPortPosition(fromPosition, connection.fromPort as 'input' | 'output' | 'tool' | 'memory');
    const toPos = getPortPosition(toPosition, connection.toPort as 'input' | 'output' | 'tool' | 'memory');
  ...
  const renderCanvasContent = () => (
    <Canvas
      ...
      onNodeSelect={(id) => { if (id) handleSelectionNodeClick(id); else clearSelection(); }}
      onNodeMove={() => {}}
      onConnectionStart={() => {}}
      onConnectionEnd={() => {}}
    ...
    >
      {selectionBox.isActive && (
        <SelectionBox startX={selectionBox.startX} startY={selectionBox.startY} currentX={selectionBox.currentX} currentY={selectionBox.currentY} />
      )}
  ...
  return (
    <div className="h-screen flex flex-col bg-dark">
      ...
      <div className="flex-1 relative overflow-hidden">
        {chatMode === 'docked' ? (
          <ResizablePanelGroup direction="horizontal" className="h-full">
            ...
            <ResizablePanel defaultSize={35} minSize={25}>
              <ChatPanel mode="docked" onModeChange={setChatMode} agentNode={nodes.find(n => n.type === 'ai-agent') || null} />
            </ResizablePanel>
          </ResizablePanelGroup>
        ) : (
          <div ref={canvasRef} className="w-full h-full relative">
        ...
        {chatMode === 'popup' && (
          <ChatPanel mode="popup" onModeChange={setChatMode} agentNode={nodes.find(n => n.type === 'ai-agent') || null} />
        )}
        ...
        <NodeSearchPalette isOpen={isNodePaletteOpen} onClose={() => setIsNodePaletteOpen(false)} onSelectNode={handleAddNode} />

        {menuState.isOpen && (
          <ContextMenu
            position={menuPosition}
            items={menuState.type === 'node' && menuState.targetId ? getNodeMenuItems(menuState.targetId) : getCanvasMenuItems()}
            onClose={closeMenu}
          />
        )}
