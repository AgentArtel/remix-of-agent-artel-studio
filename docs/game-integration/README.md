...
This folder holds **all instructions and prompts for Lovable** (Kimi RPG Studio) that relate to wiring Studio to the game's database (kimi-rpg / Open-RPG). Keeping them here keeps Studio work separate from the game repo's own task system (`.ai/tasks/` in the game repo).
...
- The **game repo** (kimi-rpg / Open-RPG) owns: migrations (including `011_studio_cross_schema_access.sql`), schema docs, and game code.
...
This folder lives under `docs/studio-reference/` in the game repo (a git subtree of the Studio repo). To have the same instructions in the **actual** Kimi RPG Studio repo (the one Lovable pushes to), either:
